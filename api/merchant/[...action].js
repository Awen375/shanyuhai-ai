...
export default async function handler(req, res) {
    try {
        ...
        if (req.method === 'POST' && action === 'login') {
            ...
            if (!raw) return res.status(401).json({ error: '账号或密码错误' });
        ...
        if (action === 'info') {
            ...
            const raw = await redis.get(`merchant:${id}`);
            if (!raw) return res.status(401).json({ error: '登录信息失效' });
            const merchant = typeof raw === 'string' ? JSON.parse(raw) : raw;
        ...
        if (action === 'flow') {
            ...
            for (const key of keys) {
                ...
                if (rawFlow) {
                    flows.push(typeof rawFlow === 'string' ? JSON.parse(rawFlow) : rawFlow);
                }
            }
        ...
        if (req.method === 'POST' && action === 'change-password') {
            ...
            const raw = await redis.get(`merchant:${id}`);
            if (!raw) return res.status(401).json({ error: '当前登录已失效' });
            const merchant = typeof raw === 'string' ? JSON.parse(raw) : raw;
        ...
        if (req.method === 'POST' && action === 'save-settings') {
            ...
            await redis.set(`merchant:${id}:settings`, JSON.stringify(settings));
            return res.status(200).json({ success: true });
        }
        ...
        if (action === 'styles') {
            ...
            const settingsRaw = await redis.get(`merchant:${merchant}:settings`);
        ...
        // 价目（公开）
        if (action === 'pricing') {
            const pricingRaw = await redis.get('config:pricing');
            const pricing = pricingRaw ? (typeof pricingRaw === 'string' ? JSON.parse(pricingRaw) : pricingRaw) : {
                items: [
                    { amount: 10, price: '¥1' },
                    { amount: 50, price: '¥5' },
                    { amount: 100, price: '¥9' },
